...
public class DownloadPage extends DecoratorAnimatedPage implements DecoratorPage {
    ...
    private final TabHeader.Tab<VBox> bedrockTab = new TabHeader.Tab<>("bedrockTab");  // 新增基岩版标签页
    ...
    public DownloadPage(String uploadVersion) {
        ...
        // 新增基岩版标签页
        bedrockTab.setNodeSupplier(() -> createBedrockPage());

        tab = new TabHeader(transitionPane, newGameTab, modpackTab, modTab, resourcePackTab, shaderTab, worldTab, bedrockTab);
        ...
        AdvancedListBox sideBar = new AdvancedListBox()
                ...
                .addNavigationDrawerTab(tab, bedrockTab, i18n("download.bedrock"), SVG.PUBLIC);  // 新增基岩版菜单
     ...
     * 创建基岩版提示页面
     */
    private VBox createBedrockPage() {
        VBox box = new VBox(25);
        box.setAlignment(Pos.CENTER);
        box.setPadding(new Insets(50));

        Label title = new Label("🧱 " + i18n("download.bedrock.title"));
        title.setStyle("-fx-font-size: 28px; -fx-font-weight: bold;");

        Label desc = new Label(i18n("download.bedrock.desc"));
        desc.setStyle("-fx-font-size: 14px; -fx-text-fill: #aaa;");
        desc.setTextAlignment(TextAlignment.CENTER);
        desc.setWrapText(true);

        JFXButton openButton = new JFXButton(i18n("download.bedrock.open_mcappx"));
        openButton.setStyle("-fx-font-size: 16px; -fx-background-color: #4caf50; -fx-text-fill: white; -fx-padding: 10 20;");
        openButton.setOnAction(e -> {
            try {
                java.awt.Desktop.getDesktop().browse(new URI("https://www.mcappx.com"));
                Controllers.showToast(i18n("download.bedrock.opening"));
            } catch (IOException | URISyntaxException ex) {
                Controllers.dialog(i18n("download.bedrock.open_failed") + "\n" + ex.getMessage(),
                    i18n("message.error"),
                    MessageDialogPane.MessageType.ERROR);
            }
        });

        Label warning = new Label("⚠️ " + i18n("download.bedrock.warning"));
        warning.setStyle("-fx-font-size: 12px; -fx-text-fill: #ffaa66;");

        box.getChildren().addAll(title, desc, openButton, warning);
        return box;
    }
